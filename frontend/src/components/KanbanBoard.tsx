...
export const KanbanBoard = () => {
  ...
  const totalCards = board ? Object.keys(board.cards).length : 0;
  ...
  return (
    <div className="flex min-h-screen w-full">
      ...
      <div className="relative flex min-w-0 flex-1 flex-col">
        ...
        <header className="relative z-10 flex items-center justify-between gap-4 border-b border-[var(--stroke)] bg-white/70 px-6 py-4 backdrop-blur">
          <div className="flex items-baseline gap-3">
            ...
            <span className="hidden text-xs font-medium text-[var(--gray-text)] sm:inline">
      ...
      {sidebarOpen && (
        <AIChatSidebar onRefresh={refresh} onClose={() => setSidebarOpen(false)} />
      )}
    </div>
  );
};
