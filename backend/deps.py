...
def get_default_board(user: User, db: Session) -> Board:
    ...
    board = db.query(Board).filter_by(user_id=user.id).order_by(Board.id).first()
    if not board:
...
def effective_role(board: Board, user: User, db: Session) -> Role | None:
    ...
    if board.user_id == user.id:
        return "owner"
    membership = db.query(BoardMembership).filter_by(
        board_id=board.id, user_id=user.id
    ).first()
    return membership.role if membership else None  # type: ignore[return-value]


def _board_with_role(board_id: int, user: User, db: Session, required: Role) -> Board:
